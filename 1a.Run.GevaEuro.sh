#!/usr/bin/env bash
#SBATCH -J geva_europe # A single job name for the array
#SBATCH --ntasks-per-node=15 # one core
#SBATCH -N 1 # on one node
#SBATCH -t 1-00:00
#SBATCH --mem 50G
#SBATCH -o /project/berglandlab/connor/err/geva_europe.%A_%a.out # Standard output
#SBATCH -e /project/berglandlab/connor/err/geva_europe.%A_%a.err # Standard error
#SBATCH -p standard
...
bgzip ${name}.SNPannotated.forGEVA.vcf
tabix ${name}.SNPannotated.forGEVA.vcf.gz
...
# Run shapeit
shapeit4 \
--input ${name}.SNPannotated.forGEVA.vcf.gz \
--region ${chrom} \
-T ${CPU} \
-O ${name}.phased.vcf
